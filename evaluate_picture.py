...
def save_sample(img, msk, msk_pred, th=0.3, name=''):
    ...
    plt.imshow(img2/255.)
    plt.axis('off')
    plt.savefig('./results/' +"image"+ name+ '.png')

    #plt.subplot(3,1,2)
    plt.imshow(msk2*255, cmap= 'gray')
    plt.axis('off')
    plt.savefig('./results/' + "mask"+name + '.png')

    #plt.subplot(3,1,3)
    plt.imshow(mskp*255, cmap = 'gray')
    plt.axis('off')

    plt.savefig('./results/'+"mask_pred"+name+'.png')

    #je picture
    imagg = cv2.imread('./results/' +"image"+ name+ '.png')
    maskk = cv2.imread('./results/' + "mask"+name + '.png')
    maskk_pred = cv2.imread('./results/'+"mask_pred"+name+'.png')
    x1 = cv2.add(imagg,maskk)
    x2 = cv2.add(imagg,maskk_pred)
    cv2.imwrite('./results/'+"results"+name+'.png',x1)
    cv2.imwrite('./results/'+"results_pred"+name+'.png',x2)
...
# F1 score (F-measure) or DSC: 0.9022765944091553
# Accuracy: 0.9622844696044922
# Specificity: 0.9732782992286638
# Sensitivity: 0.9121644131100778
# val_mode
