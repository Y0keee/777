...
def save_sample(imgfile, th=0.3, outputname=''):
    ...
    # 5. 保存图像
    cv2.imwrite(os.path.join(name+"maskpred"+ext),mskp)
    cv2.imwrite(os.path.join(name+"canny"+ext), cannymskp)
    cannymskp = cv2.imread(os.path.join(name+"canny"+ext))
...
def image():
    ...
    path_file_number = glob.glob('./image/*.png')
    j = len(path_file_number)
    file_names = []
    mask_names = []
    ...
    for file in uploaded_files:
        if 'file' not in request.files:
            return render_template('image.html', msg='上传失败，请重新选择！')
        if file and allowed_file(file.filename):
            # 保存上传的文件
            file_name = '.\\static\\' + str(j) + '.png'
            file.save(file_name)
            sava_file = '.\\static\\' + str(j) + '_pred.png'
            # 使用save_sample函数处理上传的文件
            save_sample(file_name, th=0.3, outputname=sava_file)

            # 记录文件和掩模路径
            file_names.append(file_name)
            mask_names.append(sava_file)
            j += 1
        else:
...
def image1():
    ...
    '''
        uploaded_files = request.files.getlist("file[]")
        path_file_number = glob.glob('./static/images1/*.dcm')
        j = len(path_file_number)
        for file in uploaded_files:
            if file and allowed_file(file.filename):
                test_x = []
                file_name = os.path.join(app.config['UPLOAD_FOLDER'], str(j) + '.dcm')
                file.save(file_name)
    '''
    # 获取上传的PNG文件
    uploaded_files = request.files.getlist("file[]")
    ...
    for uploaded_file in uploaded_files:
        if uploaded_file and allowed_file(uploaded_file.filename):
            ...
            # 使用save_sample函数处理上传的文件
            save_sample(file_name, th=0.3, outputname='image1')

            # 记录文件和掩模路径
            file_path = '../static/image1.png'
            mask_path = '../static/image_pred1.png'

            # 使用上传的文件和掩模路径渲染模板
            return render_template('image1.html', msg='上传成功', FILE_PATH=file_path, MASK_PATH=mask_path)
        else:
